document.addEventListener('DOMContentLoaded', function () {
  ...
  function searchRestaurants(query) {
        ...
        query: {
          bool: {
            should: [
              ...
              {match: {custom_category: {query: query, boost: 3}}},
              {
                nested: {
                  path: "menus",
                  query: {
                    bool: {
                      should: [
                        {match: {"menus.menu_name": {query: query, boost: 1}}},
                        {match: {"menus.description": {query: query, boost: 1}}}
                      ]
                    }
                  }
                }
    ...
    .then(response => response.json())
    .then(data => displaySearchResults(
        data.hits.hits))  // Assuming response format
    .catch(error => console.error('Error:', error));
  }
